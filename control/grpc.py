...
class GatewayService(pb2_grpc.GatewayServicer):
    ...
    def create_listener_safe(self, request, context):
        ...
        self.logger.info(f"Received request to create {request.host_name}"
                         ...
                         f" {request.traddr}:{request.trsvcid}, secure: "
                         f"{request.secure}, verify host name: {request.verify_host_name}, context: {context}{peer_msg}")
        ...
        with omap_lock:
            ...
            if request.host_name == self.host_name:
                try:
                    for secure in [False, True]:
                        ...
                        if lstnr in self.subsystem_listeners[request.nqn]:
                            errmsg = f"{create_listener_error_prefix}: Subsystem already listens on address {request.traddr}:{request.trsvcid}"
                            self.logger.error(errmsg)
                            return pb2.req_status(status=errno.EEXIST, error_message=errmsg)
                ...
                self.logger.info(f"Gateway's host name \"{self.host_name}\" differs from requested one \"{request.host_name}\". Listener will be stashed to be used later by the right gateway.")
                ret = True
        ...
        if listener_created:
            ...
            return pb2.req_status(status=errno.EREMOTE, error_message="Host name mismatch, listener will only be active when the appropriate gateway is up")
